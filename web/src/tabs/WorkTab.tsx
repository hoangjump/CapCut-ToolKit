        ...
        <CardContent><Table>
          ...
          <TableBody>
            ...
            {tasks.map((task) => <TableRow key={task.id}>
              ...
              <TableCell className="max-w-[360px]"><div className="truncate">{task.description}</div><div className="text-xs text-muted-foreground">{new Date(task.createdAt).toLocaleString('vi-VN')}{task.deadline ? ` · Hạn ${new Date(task.deadline).toLocaleString('vi-VN')}` : ''}</div>{task.deliveryError && <div className="text-xs text-destructive">{task.deliveryError}</div>}</TableCell>
              <TableCell>{task.quantity} con<div className="text-xs text-muted-foreground">{formatMoney(task.unitRate)}/con</div></TableCell>
              <TableCell>{formatMoney(task.amount)}</TableCell>
              <TableCell><Badge variant={statusVariant(task.status)}>{taskStatus[task.status]}</Badge></TableCell>
              <TableCell><div className="flex justify-end gap-1">
                {task.status === 'pending' && <><Button size="icon" variant="ghost" title="Sửa" onClick={() => setEditing(task)}><Pencil /></Button><Button size="icon" variant="ghost" title="Hoàn thành thủ công" onClick={() => taskAction(() => workApi.completeTask(task.id), 'Đã hoàn thành')}><Check /></Button><Button size="icon" variant="ghost" title="Hủy" onClick={() => taskAction(() => workApi.cancelTask(task.id), 'Đã hủy')}><X /></Button></>}
                {task.status === 'completed' && <Button size="icon" variant="ghost" title="Mở lại" onClick={() => taskAction(() => workApi.reopenTask(task.id), 'Đã mở lại công việc')}><RotateCcw /></Button>}
                {task.status === 'failed' && <><Button size="icon" variant="ghost" title="Gửi lại" onClick={() => taskAction(() => workApi.retryTask(task.id), 'Đã gửi lại công việc')}><RefreshCw /></Button><Button size="icon" variant="ghost" title="Hủy" onClick={() => taskAction(() => workApi.cancelTask(task.id), 'Đã hủy')}><X /></Button></>}
              </div></TableCell>
            </TableRow>)}
          </TableBody>
        </Table></CardContent>
