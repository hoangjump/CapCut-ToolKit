...
export function MailTab() {
  ...
  async function makeAliases(m: MailRecord) {
    ...
    if (!confirm(`Tạo alias cho ${m.email}?\nSẽ mở trình duyệt, đăng nhập rồi tạo tới 10 alias. Chạy nền — theo dõi ở khung log, alias hiện dần trong kho.`)) return;
    try {
      // Chạy nền: server trả 202 ngay, alias hiện dần trong kho (tự refresh 5s).
      await mailApi.createAliases(m.id);
      toast.success(`Đã bắt đầu tạo alias cho ${m.email} — xem khung log; alias sẽ hiện dần trong kho`);
    } catch (e) { toast.error((e as Error).message); }
  }
              ...
              {pageMails.map((m) => (
                <TableRow key={m.id}>
                  ...
                  <TableCell className="text-right">
                    ...
                    {m.source !== 'alias' && <Button variant="ghost" size="icon" title="Tạo alias (login + tạo tới 10 alias, chạy nền)" onClick={() => void makeAliases(m)}><Copy className="h-4 w-4" /></Button>}
